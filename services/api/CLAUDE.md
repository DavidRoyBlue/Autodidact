...
- `ApiAgentClient` (`src/services/agent.client.ts`) is the primary component that calls the Agent service HTTP API. Two sanctioned exceptions call it directly: the liveness probe in `health.controller.ts` (`GET /health` pings the agent's `/health`) and the SSE proxy in `chat.service.ts` (the streaming bridge does not fit a request/response client method). All three call sites — and only these — may reach the agent, and **every** one must attach `cloudRunAuthHeaders(agentUrl)` from `@autodidact/providers` (the agent is a private Cloud Run service). Do not add a fourth caller; prefer `ApiAgentClient` for any new non-streaming call.
