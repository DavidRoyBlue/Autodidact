  ...
  return (
    <AppText variant="body">
      {segments.map((seg, i) => {
        ...
        if (seg.type === 'code') {
          return (
            <AppText key={i} fontFamily="$mono" backgroundColor="$surfaceHover" borderRadius="$sm" paddingHorizontal="$1">
              {` ${seg.content} `}
            </AppText>
          );
        }
    ...
    if (seg.type === 'codeblock') {
      ...
      nodes.push(
        <YStack key={k++} backgroundColor="$surfaceHover" borderRadius="$sm" padding="$3" marginTop="$2">
          <AppText fontFamily="$mono" variant="body" size="sm">{seg.content.trim()}</AppText>
        </YStack>,
      );
    } else {
      buf.push(seg);
