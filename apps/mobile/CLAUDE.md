...
- **Social sign-in (Spec 2 Phase 1):** Google (native `signInWithIdToken`) and Facebook (web-PKCE `openAuthSessionAsync`) sign-in routes through [`src/lib/social-auth.ts`](./src/lib/social-auth.ts). The app must run on a **custom dev build** (not Expo Go); native Google sign-in cannot run in the Expo sandbox. Guest→OAuth upgrade is deferred to Phase 2. See [`docs/social-sign-in.md`](./docs/social-sign-in.md) for setup and dual Android client ID requirements.
