...
- **Social sign-in (Spec 2 Phases 1–2):** Google (native `signInWithIdToken`) and Facebook (web-PKCE `openAuthSessionAsync`) sign-in routes through [`src/lib/social-auth.ts`](./src/lib/social-auth.ts). The app must run on a **custom dev build** (not Expo Go); native Google sign-in cannot run in the Expo sandbox. Guest→OAuth upgrade is **live** via `linkWithGoogle()`/`linkWithFacebook()` (web `linkIdentity` flow); requires **manual linking** enabled in Supabase auth settings. The `0011` identity-link trigger ensures `public.users` stays synced. See [`docs/social-sign-in.md`](./docs/social-sign-in.md) for setup and dual Android client ID requirements.
