...
- **D5 — Anonymous sign-in is ENABLED (production onboarding).** `signInAnonymously()` for low-friction entry; upgrade via `updateUser({email})`/`linkIdentity()` **preserves the user UUID**, so the provisioned `public.users` row + progress carry over. Mitigations: CAPTCHA/Turnstile, IP rate-limit, and a stale-anonymous-user cleanup job. Distinct from DEV_AUTO_LOGIN (Spec 4).
...
- **GoTrue settings via `supabase/config.toml`** (Spec 1 — config-as-code, mirrored to prod per Spec 1's still-open `config push` question): require email confirmation **for email identities**; **ENABLE anonymous sign-ins** (D5) with CAPTCHA/Turnstile + tuned IP rate-limit; leaked-password protection (HIBP); password min length/strength; Site URL + redirect allow-list; evaluate MFA. *(Settings only — not DDL.)*
...
**GoTrue settings** (email confirmation, anonymous toggle, rate limits, CAPTCHA, password policy, redirect allow-list) are **not DDL** — they live in `supabase/config.toml` (Spec 1) for local, mirrored to prod (Spec 1's `config push` parity question). They never go in a migration.
...
- `supabase/config.toml` — GoTrue settings (anonymous enabled, email confirmation, rate limits, CAPTCHA, password policy, redirect allow-list).
