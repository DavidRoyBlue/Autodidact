...
- **Revisit when the #84 redesign lands a multi-node generation graph** (research/fan-out/per-module content). At that point Studio's step-through debugging and checkpoint inspection earn their keep; budget the `langgraph.json` + graph-export refactor into that work. The target design is [ADR-030](ADRs/services/agent/ADR-030-course-generation-graph-execution-model.md) + its spec, which schedules the Studio wiring with the plan/loop step (#92).
