...
- Prompts must be exported with descriptive names that make their purpose unambiguous: `COMPLETION_EVALUATOR_SYSTEM_PROMPT`, `buildModuleSystemPrompt`.
...
- Dynamic prompts that incorporate runtime data (module content, user context, objectives list) are exported as builder functions (`buildXxxPrompt(...)`).
